...
import type { AnimationStyle, GlowProps, GradientStop, Layout } from "./types";
...
export const Glow: React.FC<GlowProps> & React.FunctionComponent<GlowProps> = memo<GlowProps>(
  (props: GlowProps): React.ReactElement | null => {
    const {
      children,
      size = 4,
      color = "#8b5cf6",
    ...
    useEffect(() => {
      ...
      const animations: Record<AnimationStyle, number> = {
        linear: withRepeat(
          withTiming(1, {
        ...
        withoutEasing: withRepeat(withTiming(1, { duration: adjustedDuration }), -1, false),
        pulse: withRepeat(
          withTiming(1, {
            duration: adjustedDuration,
      ...
      // Ensure we always assign a number (fallback to 0) to avoid undefined assignment
      // (some animation keys/values may not be present at compile-time under certain moduleResolution settings)
      progress.value = (animations as Record<string, number>)[style] ?? 0;
    ...
    const animatedOpacity = useAnimatedProps(() => {
      ...
      const baseOpacity =
        style === "breathe" ? progress.value : interpolate(progress.value, [0, 0.5, 1], [0.6, 1, 0.6]);
      ...
      <View style={{ position: "relative" }}>
        {enabled && (
          <View
            ...
            pointerEvents="none">
            <Svg width={layout.width + size * 2} height={layout.height + size * 2}>
              <Defs>
                <AnimatedLinearGradient id="gradient" animatedProps={animatedGradient}>
                  {stops.map((stop: GradientStop, index: number) => (
...
export default memo<React.FC<GlowProps> & React.FunctionComponent<GlowProps>>(Glow);
