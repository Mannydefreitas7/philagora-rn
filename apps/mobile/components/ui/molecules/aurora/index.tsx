...
import { useSharedValue, useFrameCallback, useDerivedValue, type FrameInfo } from "react-native-reanimated";
...
export const Aurora: React.FC<IAurora> & React.FunctionComponent<IAurora> = memo<IAurora>(
  ({
    width: paramsWidth,
    height: paramsHeight,
    ...
    intensity = 0.4,
    waveDirection = [-20, 20],
  }: React.ComponentProps<typeof Aurora>): (React.JSX.Element & React.ReactNode & React.ReactElement) | null => {
    ...
    const color1 = useMemo(() => hexToRgb<string>(auroraColors[0] ?? DEFAULT_AURORA_COLORS[0]), [auroraColors]);
    const color2 = useMemo(() => hexToRgb<string>(auroraColors[1] ?? DEFAULT_AURORA_COLORS[1]), [auroraColors]);
    const color3 = useMemo(() => hexToRgb<string>(auroraColors[2] ?? DEFAULT_AURORA_COLORS[2]), [auroraColors]);
    const skyTop = useMemo(() => hexToRgb<string>(skyColors[0]), [skyColors]);
    const skyBottom = useMemo(() => hexToRgb<string>(skyColors[1]), [skyColors]);
...
export default memo<React.FC<IAurora> & React.FunctionComponent<IAurora>>(Aurora);
