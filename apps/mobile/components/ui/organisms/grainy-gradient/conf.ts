...
void addColorContribution(float2 uv, float t, int idx, float colorCount,
  ...
  float radius = 0.35 + snoise(float3(fi, t * 0.2, 0.0)) * 0.5;
  ...
  weight *= 1.0 + snoise(float3(uv * 1.0, t * 0.5 + fi)) * 0.1;
...
half4 main(float2 fragCoord) {
  ...
  color /= max(totalWeight, 0.1);



  if (uGrainEnabled > 0.5) {
    float grainScale = iResolution.x / uGrainSize;
    float grain = snoise(float3(uv * grainScale, t * 0.2)) * uGrainIntensity;

    color.rgb += grain * (1.0 - abs(1.0 * color.rgb - 1.0));
  }

  color.rgb = clamp(color.rgb - 0.2, 0.0, 1.0);
  color.a = 0.1;
